...
process.on('uncaughtException', (err) => {
  console.error('[azteca-quick] Uncaught exception:', err.message);
});
process.on('unhandledRejection', (reason) => {
  console.error('[azteca-quick] Unhandled rejection:', reason);
});
...
app.setAsDefaultProtocolClient('azteca-quick');
...
app.whenReady().then(() => {
  ...
  ipcMain.on('open-external', (_e, url) => {
    if (typeof url === 'string' && url.startsWith('https://aztecaapp.com')) {
  ...
  const protocolArg = process.argv.find(a => a.startsWith('azteca-quick://'));
...
function createTray() {
  ...
  tray = new Tray(icon);
  tray.setToolTip(`Quick by Azteca (${shortcut})`);
  ...
  const contextMenu = Menu.buildFromTemplate([
    ...
    { label: 'Reiniciar', click: () => {
      if (win && !win.isDestroyed()) win.loadURL('https://aztecaapp.com/exam');
    }},
    { type: 'separator' },
...
function createWindow() {
  ...
  win.loadURL('https://aztecaapp.com/exam');
  ...
  win.webContents.setWindowOpenHandler(({ url }) => {
    if (url.includes('aztecaapp.com/exam')) {
      win.loadURL(url);
    } else {
      require('electron').shell.openExternal(url);
    }
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .link:hover { background: #008c44; }
  .shortcut { font-size: 11px; color: #555; margin-top: 24px; }
</style>
</head>
<body>
  <div class="logo"><span>quick</span></div>
  <div class="subtitle">by Azteca</div>
  <div class="instructions">
    <div class="step">Para conectar tu cuenta:</div>
    <div class="step">1. Abre <strong>aztecaapp.com</strong> en tu navegador</div>
    ...
    <a class="link" href="https://aztecaapp.com" target="_blank">Abrir aztecaapp.com</a>
  </div>
...
function injectUI() {
  ...
  win.webContents.executeJavaScript(`
    (function() {
      ...
      if (!document.getElementById('azteca-bar')) {
      ...
      // --- Escape to hide ---
      if (!window._aztecaKeys) {
        window._aztecaKeys = true;
        document.addEventListener('keydown', function(e) {
          if (e.key === 'Escape' && window.azteca) window.azteca.hide();
        });
      }

      // --- Drag & drop images ---
      if (!window._aztecaDrop) {
        window._aztecaDrop = true;
        ...
        document.addEventListener('dragenter', function(e) {
          e.preventDefault();
          if (!document.getElementById('azteca-drop-overlay')) {
            var overlay = document.createElement('div');
            overlay.id = 'azteca-drop-overlay';
            overlay.style.cssText = 'position:fixed;inset:0;z-index:99998;background:rgba(0,166,81,0.1);border:2px dashed rgba(0,166,81,0.5);border-radius:16px;display:flex;align-items:center;justify-content:center;pointer-events:none';
            overlay.innerHTML = '<div style="background:rgba(0,0,0,0.7);padding:8px 16px;border-radius:10px;color:#00A651;font-size:13px;font-weight:600">Suelta la imagen aqui</div>';
        ...
        document.addEventListener('dragleave', function(e) {
          if (e.relatedTarget === null || e.relatedTarget === document.documentElement) {
            var ov = document.getElementById('azteca-drop-overlay');
            if (ov) ov.remove();
          }
        });
...
async function toggleWindow() {
  ...
  // 2. Now show the Azteca window at saved position (or default)
  const saved = loadBounds();
  const defaults = getDefaultPosition();
  const bounds = saved || { x: defaults.x, y: defaults.y, width: 340, height: 280 };
...
app.on('second-instance', (_event, argv) => {
  ...
  const protocolArg = argv.find(a => a.startsWith('azteca-quick://'));
  if (protocolArg) {
...
function handleProtocolUrl(url) {
  try {
    if (typeof url !== 'string' || !url.startsWith('azteca-quick://')) {
      console.warn('[azteca-quick] Invalid protocol URL format');
      return;
    }

    const parsed = new URL(url);
    if (parsed.protocol !== 'azteca-quick:') return;

    const refreshToken = parsed.searchParams.get('rt');
    if (!refreshToken || refreshToken.length < 10 || refreshToken.length > 4096) {
      console.warn('[azteca-quick] Invalid or missing refresh token');
      return;
    }
    ...
    if (!sanitized || sanitized.length < 10) {
      console.warn('[azteca-quick] Token failed sanitization');
      return;
    }

    win.loadURL(`https://aztecaapp.com/exam?quick_auth=${encodeURIComponent(sanitized)}`);
    if (panelModule) {
      win.showInactive();
      panelModule.makeKeyWindow(win);
    } else {
    ...
    console.error('[azteca-quick] Protocol handler error:', e.message);
  }
}
